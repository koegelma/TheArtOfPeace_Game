fixed GEN_BASE(float3 p, float4 params)
{
	fixed final = 0;
	...
	final = lerp(shapeC, shapeD, params.w);

	return final;
}

//---m = transform matrix (pos,rot,scale)
fixed4 GENERATOR_SHAPE(fixed3 center, fixed3 interFinalColor, fixed4 sizes, float4x4 m)
{
	fixed4 q = mul(m, fixed4(center, 1));
	center = q.xyz;

	return fixed4(interFinalColor, GEN_BASE(center, sizes));
}